...
const BotaoChek1 = BotaoChek

export function ListarProdutos({ recarregar, setRecarregar, onEdit }: ListarProdutosProps) {
    ...
    const [mostrarApenasArtesanal, setMostrarApenasArtesanal] = useState(false);
        ...
        <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 m-5">
                {filtrarProdutos().map((produto, index) => (
                    <CardProduto key={produto.id} item={index} produto={produto} onEdit={onEdit} onDelete={handleDelete} />
                ))}
            </div>
        </>
    )
